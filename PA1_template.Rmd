...
First let's calculate the total number of steps for each day. There are missing values on some days, but let's ignore that for now.  

```{r}
steps_per_day <- sapply(split(activity, activity$date), function(x) (sum(x[, 1])))
```

steps_per_day contains the number of steps each day over the two months period. We can directly compute the mean and median and plot a histogram from this variable:  
...
interval <- strptime(sprintf("%04s", names(daily_average)), format = "%H%M")

plot(daily_average ~ interval, type = "l")
```
